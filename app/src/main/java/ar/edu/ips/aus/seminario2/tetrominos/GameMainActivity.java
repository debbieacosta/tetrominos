...
public class GameMainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Bundle data_level = getIntent().getExtras();
            if(data_level!=null){
                level = data_level.getInt("level");
            }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case GAME_FINISHED:
                    ...
                    int myCurrentScore = Integer.parseInt(scoreView.getText().toString());

                    SharedPreferences saveScore = getPreferences(MODE_PRIVATE);
                    int highScore = saveScore.getInt("HIGH_SCORE", 0);
                  // SharedPreferences.Editor editor = saveScore.edit();

                    if(myCurrentScore > highScore){
                        SharedPreferences.Editor editor = saveScore.edit();
                        editor.putInt("HIGH_SCORE", myCurrentScore);
                        editor.commit();
                        newScore = 1;
                    }

                    GameOverDialog gameOverDialog = new GameOverDialog(GameMainActivity.this, newScore);
